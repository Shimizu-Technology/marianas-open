...
The Marianas Open website supports 7 languages:
...
3. GT's AI translation engine generates accurate translations for all 6 target languages
4. The translated files (`ja.json`, `ko.json`, `zh.json`, `zh-Hant.json`, `tl.json`, `pt.json`) are written in-place
...
│       ├── zh.json             # Auto-generated by GT CLI (Simplified Chinese)
│       ├── zh-Hant.json        # Auto-generated by GT CLI (Traditional Chinese)
...
```json
{
  ...
  "defaultLocale": "en",
  "locales": ["ja", "ko", "zh", "zh-Hant", "tl", "pt"],
  "customMapping": {
...
5. Verify: check that `ja.json`, `ko.json`, `zh.json`, `zh-Hant.json`, `tl.json`, `pt.json` have been updated
...
- **Starter ($5/mo)**: Includes $10 usage credit. Covers ~560 keys across 6 target languages with frequent deploys. GT only re-translates changed keys, keeping costs minimal.
...
```
  Admin saves event ──► Rails API ──► Save to DB (English)
                                   ...
                                   (calls GT API for 6 target locales)
                                          │
                                          ▼
                                   Save translations
...
event.translations["name"]["zh-Hant"]  # => "關島 Copa de Marianas 2026"
...
4. Job calls `GtTranslationService.translate_fields` for each target locale (ja, ko, zh, zh-Hant, tl, pt)
