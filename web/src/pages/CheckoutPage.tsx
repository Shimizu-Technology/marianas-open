          ...
          <div>
            ...
            {method === 'pickup' && pickup && (
              <section className="mt-6 rounded-2xl border border-white/10 bg-surface p-5 sm:p-7">
                <div className="flex gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gold/10 text-gold"><MapPin className="h-5 w-5" /></div><div><h2 className="font-heading text-xl font-semibold">{pickup.name}</h2><div className="mt-2 text-sm leading-6 text-text-secondary">{formatAddress(pickup.address).map(line => <div key={line}>{line}</div>)}</div>{!pocMode && pickup.phone && <a className="mt-2 inline-flex text-sm text-gold hover:text-gold-300" href={`tel:${pickup.phone}`}>{pickup.phone}</a>}</div></div>
                {pocMode ? <div className="mt-5 rounded-xl bg-white/[0.04] p-4 text-sm leading-6 text-text-secondary">Demo pickup only. No merchandise will be held or prepared for pickup.</div> : pickup.pickup_instructions && <div className="mt-5 rounded-xl bg-white/[0.04] p-4 text-sm leading-6 text-text-secondary">{pickup.pickup_instructions}</div>}
                <div className="mt-6 border-t border-white/10 pt-6"><p className="text-xs font-bold uppercase tracking-[0.18em] text-gold">Pickup contact</p><h3 className="mt-2 font-heading text-lg font-semibold">{pocMode ? 'Who is trying the demo?' : 'Who should we notify?'}</h3><div className="mt-5 grid gap-5 sm:grid-cols-2"><Field label="Full name"><input className={inputClass} autoComplete="name" value={contact.name} onChange={event => updateContact('name', event.target.value)} /></Field><Field label="Email"><input className={inputClass} type="email" autoComplete="email" value={contact.email} onChange={event => updateContact('email', event.target.value)} /></Field><div className="sm:col-span-2"><Field label="Phone" optional><input className={inputClass} type="tel" autoComplete="tel" value={contact.phone} onChange={event => updateContact('phone', event.target.value)} /></Field></div></div></div>
              </section>
            )}
