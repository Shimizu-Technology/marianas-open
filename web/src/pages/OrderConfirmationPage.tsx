    ...
    <div className="min-h-screen px-4 pb-24 pt-32 sm:px-6 sm:pt-36">
      <div className="mx-auto max-w-4xl">
        <section className="rounded-3xl border border-white/10 bg-surface p-6 sm:p-9">
          ...
          {order.status === 'paid' && <div className="mt-6 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.055] p-5"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-300">What happens next</p><span className="rounded-full border border-emerald-300/20 px-2.5 py-1 text-[11px] font-semibold text-emerald-100">{fulfillmentLabel(order.fulfillment_status)}</span></div><p className="mt-2 text-sm leading-6 text-text-secondary">{nextStep(order)}</p><p className="mt-3 text-xs text-text-muted">Keep this status link for future updates. Confirmations are addressed to {order.customer_email}.</p></div>}
          {order.refund_status !== 'none' && <div role="status" className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[0.055] p-5"><p className="text-xs font-bold uppercase tracking-[0.18em] text-amber-200">Refund update</p><p className="mt-2 text-sm leading-6 text-text-secondary">{order.refund_status === 'pending' ? 'A refund is being processed by Stripe. This page will reflect it when the refund completes.' : `${money(order.refunded_cents, order.currency)} has been refunded to the original payment method. Your bank may take several business days to display it.`}</p></div>}
