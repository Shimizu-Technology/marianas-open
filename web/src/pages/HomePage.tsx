            ...
            <ScrollReveal delay={0.15}>
              {merchandisePreview.length > 0 ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:gap-5">
                  {merchandisePreview.map((product) => {
                    const inStock = product.variants.some(variant => variant.available_quantity > 0);
                    return (
                      <Link
                        key={product.id}
                        to={`/shop/${product.slug}`}
                        className="group relative block overflow-hidden rounded-2xl border border-white/10 bg-surface focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
                      >
