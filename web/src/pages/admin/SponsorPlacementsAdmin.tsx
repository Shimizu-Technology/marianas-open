      ...
      <section className="divide-y divide-white/5 border border-white/5 bg-surface">
        {placements.length === 0 ? <div className="p-8 text-center text-sm text-text-muted">No placements yet.</div> : placements.map((placement) => (
          <div key={placement.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2"><label className="cursor-pointer p-2 text-text-muted hover:text-gold" title={placement.placement_type === 'featured_bar' ? 'Upload custom image' : 'Upload custom image or video'}><ImageUp className="h-4 w-4" /><input type="file" accept={placement.placement_type === 'featured_bar' ? 'image/jpeg,image/png,image/webp,image/gif' : 'image/*,video/mp4,video/webm'} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) uploadMedia(placement, file); event.target.value = '' }} /></label><button onClick={() => toggle(placement)} className={`px-3 py-1.5 text-xs ${placement.active ? 'bg-green-400/10 text-green-300' : 'bg-white/5 text-text-muted'}`}>{placement.active ? 'Active' : 'Inactive'}</button><button onClick={() => remove(placement)} className="p-2 text-text-muted hover:text-red-300" aria-label="Delete placement"><Trash2 className="h-4 w-4" /></button></div>
          </div>
        ))}
      </section>
    </div>
