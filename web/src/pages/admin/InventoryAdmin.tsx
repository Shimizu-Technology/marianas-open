...
export default function InventoryAdmin() {
  ...
  return <div className="mx-auto max-w-4xl p-4 sm:p-6 lg:p-8">
    ...
    {loading ? <div className="mt-8 flex h-48 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-gold" /></div> : !snapshot?.products.length || !snapshot.locations.length ? <div className="mt-8 rounded-2xl border border-white/10 bg-surface p-6 text-sm text-text-secondary">No products with variants or active stock locations are available yet. Ask a merchandise admin to set them up.</div> : <form onSubmit={event => void record(event)} className="mt-8 space-y-5 rounded-2xl border border-white/10 bg-surface p-5 sm:p-7">
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="text-sm font-semibold">Product<select className={`${inputClass} mt-2`} value={productId} onChange={event => { setProductId(event.target.value); setVariantId('') }}>{snapshot.products.map(item => <option key={item.id} value={item.id}>{item.name}{item.active ? '' : ' · Not live'}</option>)}</select></label>
        <label className="text-sm font-semibold">Variant<select className={`${inputClass} mt-2`} value={variant?.id || ''} onChange={event => setVariantId(event.target.value)}>{variants.map(item => <option key={item.id} value={item.id}>{item.name} · {item.sku}{item.active ? '' : ' · Inactive'}</option>)}</select></label>
