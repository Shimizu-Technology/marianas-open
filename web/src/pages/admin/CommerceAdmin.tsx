...
export default function CommerceAdmin() {
  ...
  const assignImage = async (imageId: number, variantId: number | null) => {
    if (!draft?.id) return
    setSaving(true); setError(''); setNotice('')
    try {
      const response = await api.admin.assignProductImage(draft.id, imageId, variantId)
      const saved = normalized(response.product)
      setDraft(saved); setProducts(current => current.map(product => product.id === saved.id ? saved : product))
      const variant = saved.variants.find(item => item.id === variantId)
      setNotice(variant ? `Photo assigned to ${variant.name}.` : 'Photo now applies to all variants.')
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Photo assignment could not be saved.') }
    finally { setSaving(false) }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        ...
        <ProductImagesSection product={draft} saving={saving} onUpload={file => void uploadImage(file)} onDelete={imageId => void deleteImage(imageId)} onAssign={(imageId, variantId) => void assignImage(imageId, variantId)} />
      </div>
