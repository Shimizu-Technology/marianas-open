...
function combinationCount(options: ProductOption[]) {
  ...
  return options.reduce((total, option) => {
    ...
    return total > MAX_VARIANTS / option.values.length ? MAX_VARIANTS + 1 : total * option.values.length
  }, 1)
}
...
export default function CommerceAdmin() {
  ...
  const plannedVariantCount = draft ? combinationCount(draft.options) : 0
  ...
  const generateVariants = () => setDraft(current => {
    ...
    const count = combinationCount(current.options)
    if (count > MAX_VARIANTS) {
      setError(`This setup creates more than ${MAX_VARIANTS} variants. Remove an option or some values before generating.`)
      return current
    }
  ...
  const save = async () => {
    ...
    if (draft.active && draft.images.length === 0) { setError('Save this as a draft, upload a product image, then publish it.'); return }
  ...
  const deleteImage = async (imageId: number) => {
    ...
    setSaving(true); setError('')
    try {
      const response = await api.admin.deleteProductImage(draft.id, imageId)
      const saved = normalized(response.product)
      setDraft(saved); setProducts(current => current.map(product => product.id === saved.id ? saved : product))
      setNotice('Product image removed.')
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Image could not be removed.') }
    finally { setSaving(false) }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-white/10 bg-surface p-5 sm:p-7"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-gold">4 · Images</p><h2 className="mt-2 font-heading text-xl font-semibold">Show customers the real product</h2><p className="mt-2 text-sm leading-6 text-text-muted">JPEG, PNG, or WebP up to 10 MB. Save as a draft, upload a photo, then publish.</p></div><label className={`${buttonSecondary} ${!draft.id || saving ? 'pointer-events-none opacity-40' : 'cursor-pointer'}`}><ImagePlus className="h-4 w-4" /> Upload image<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={!draft.id || saving} onChange={event => { const file = event.target.files?.[0]; if (file) void uploadImage(file); event.target.value = '' }} /></label></div>{draft.images.length === 0 ? <div className="mt-6 flex min-h-36 w-full items-center justify-center rounded-xl border border-dashed border-white/10 bg-black/15 sm:aspect-[16/5]"><p className="text-sm text-text-muted">No product photos yet</p></div> : <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">{draft.images.map(image => <div key={image.id} className="group relative aspect-square overflow-hidden rounded-xl border border-white/10"><img src={resolveMediaUrl(image.url) || image.url} alt={image.alt_text} className="h-full w-full object-contain bg-black/20" /><button onClick={() => void deleteImage(image.id)} className="absolute right-2 top-2 rounded-full bg-black/75 p-2 text-white opacity-100 backdrop-blur sm:opacity-0 sm:group-hover:opacity-100" aria-label="Delete image"><Trash2 className="h-4 w-4" /></button></div>)}</div>}</section>
      </div>
