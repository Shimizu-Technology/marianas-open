class OrderNotification < ApplicationRecord
  ...
  validates :kind, :recipient, :status, :provider, :idempotency_key, :subject, :html_body, :text_body,
    presence: true
  ...
  validates :recipient, uniqueness: { scope: %i[order_id kind] }
  ...
  def assign_idempotency_key
    ...
    self.idempotency_key = "marianas-open/#{Rails.env}/#{kind}/#{order_id}/#{recipient_digest}"
  end
end
